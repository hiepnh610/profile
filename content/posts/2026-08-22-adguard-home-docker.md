...
From the login screen, the two things worth changing before you trust a resolver with real traffic are under Settings. Upstream DNS servers defaults to a public resolver over plain DNS, so I point it at an encrypted upstream instead — mine is a DNS-over-HTTPS endpoint on Cloudflare Gateway. Plain DNS on port 53 is unencrypted: every query leaves AdGuard Home in the clear, and the ISP handling that hop can log every domain the house resolves no matter how much of the actual traffic afterward is wrapped in TLS. Switching that one hop to DoH doesn't change anything on the LAN side — clients still just ask AdGuard Home — but from AdGuard Home outward, the ISP now sees an HTTPS connection to Cloudflare and nothing about which domains are inside it. And under Filters, the default AdGuard blocklist is on but sparse by itself — I add one or two more lists there before pointing any real client at it, otherwise the first few days make the whole exercise feel pointless.
