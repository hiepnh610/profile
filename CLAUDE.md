...
### Third-party policy
Everything is self-hosted (`/static/fonts/`, `/static/js/`) — no CDN, no Google Fonts. The ONLY external request is GA4 (`googletagmanager.com`) in `layouts/partials/analytics.html`. Icons are inline SVG via `layouts/partials/icon.html` — do not add icon fonts.
