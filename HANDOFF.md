...
A personal site / dev blog for **Hiep Nguyen**, a software engineer based in Hanoi. Two pages:
...
- **Footer**: full-width, bordered top, mono small text. Left: copyright + tagline. Right: `Sitemap`.
...
4. **Byline** — bordered top + bottom, padding 14px. `<b>` author name · "·" separator · location · right-aligned word count. Geist Mono 11px uppercase 0.06em.
...
6. **Article footer** (`.article-foot`, bordered top, max-width 64ch):
   ...
   - **Author card** — bordered, radius 10px, gradient background. Name + bio + mono links row.
...
No images or icons — all visual elements are typography, borders, gradients, and CSS hatching. Unicode glyphs used: `→` `←` `↗` `★` `·` `↩`. If you want crisper icon control, swap to inline SVG or Lucide.
